    ...
    for (int i = format_start + 1; i < format_end; i++) {
        ...
        case 's': {
            qsizetype string_length
                = datagram->data().indexOf('\0', value_marker)
                - value_marker;
            ...
            value_marker
                = value_marker + string_length + 4 - string_length % 4;
        } break;
        case 'b': {
            qint32 blob_size = 0;
...
Osc::Message::Message(QHostAddress destinationAddress,
    quint16 destinationPort, QHostAddress sourceAddress,
    quint16 sourcePort, QString address,
...
{
    return QAbstractSocket::tr("Sender: %1:%2, Receiver: %3:%4, Address: %5, Values: %6").arg(this->sourceAddress.toString()).arg(this->sourcePort).arg(this->destinationAddress.toString()).arg(this->destinationPort).arg(this->address).arg(this->format());
}
